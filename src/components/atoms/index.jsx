...
// Rating Stars
export const Rating = ({ rate = 4.5, count = 0, size = 'md' }) => {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className={`font-bold ${sizeClass}`}>{rate}</span>
...
export const Price = ({ 
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      {original && (
        <span className={`${sizeClass} line-through text-gray-400`}>${original}</span>
      )}
      {discount && (
...
export const Spinner = ({ size = 'md' }) => {
  ...
  return (
    <div className={`${sizeClass} border-4 border-gray-300 border-t-primary rounded-full animate-spin`} />
  );
};
